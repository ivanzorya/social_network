...
class ProfileTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username=self.username, email=f"{self.username}@gmail.com", password=self.password)
        ...
        self.user_no_author = User.objects.create_user(username=self.username_not_author, email=f"{self.username_not_author}@gmail.com", password=self.password_not_author)
    ...
    def test_profile(self):
        ...
        self.assertIn(bytes(f"""<strong class="d-block text-gray-dark">@{self.user.username}</strong>""", encoding="UTF-8"), response_profile.content)
        self.assertIn(bytes(f"""{self.text}""", encoding="UTF-8"), response_profile.content)

    def test_new(self):
        response_login = self.client.get("/new")
        self.assertEqual(response_login.status_code, 200)
        self.assertIn(bytes("""<form action=" /new " method="post" enctype="multipart/form-data">""", encoding="UTF-8"), response_login.content)
        self.client.logout()
        response_not_login = self.client.get("/new", follow=True)
        self.assertEqual(response_not_login.redirect_chain, [("/auth/login/?next=/new", 302)])

    def test_post_pub(self):
        response_post_index = self.client.get("/")
        self.assertContains(response_post_index, self.cache_text[0], count=None, status_code=200, html=False)
        response_post_user = self.client.get(f"/{self.user.username}/")
        self.assertContains(response_post_user, self.text, count=None, status_code=200, html=False)
        response_post = self.client.get(f"/{self.user.username}/{self.post.pk}/")
        self.assertContains(response_post, self.text, count=None, status_code=200, html=False)
        response_post_group = self.client.get(f"/group/{self.slug}")
        self.assertContains(response_post_group, self.text, count=None, status_code=200, html=False)

    def test_post_edit(self):
        self.client.post(f"/{self.user.username}/{self.post.pk}/edit/", {"text": self.text_edit, "group": self.group.pk}, follow=True)
        edit_post = Post.objects.get(pk=self.post.pk)
        self.assertEqual(edit_post.text, self.text_edit)
        response_post_edit_index = self.client.get("/")
        self.assertContains(response_post_edit_index, self.cache_text[0], count=None, status_code=200, html=False)
        response_post_edit_user = self.client.get(f"/{self.user.username}/")
        self.assertContains(response_post_edit_user, self.text_edit, count=1, status_code=200, html=False)
        response_post_edit = self.client.get(f"/{self.user.username}/{self.post.pk}/")
        self.assertContains(response_post_edit, self.text_edit, count=1, status_code=200, html=False)
        response_post_edit_group = self.client.get(f"/group/{self.slug}")
        self.assertContains(response_post_edit_group, self.text_edit, count=1, status_code=200, html=False)
        self.client.logout()
        response_post_edit_logout = self.client.post(f"/{self.user.username}/{self.post.pk}/edit/", {"text": self.text_edit, "group": self.group.pk}, follow=True)
        self.assertEqual(response_post_edit_logout.redirect_chain, [(f"/{self.user.username}/{self.post.pk}/", 302)])
        self.client.login(username=self.username_not_author, password=self.password_not_author)
        response_post_edit_not_author = self.client.post(f"/{self.user.username}/{self.post.pk}/edit/", {"text": self.text_edit, "group": self.group.pk}, follow=True)
    ...
    def test_image(self):
        with open('media/posts/1.png', 'rb') as img:
            self.client.post(f"/{self.user.username}/{self.post.pk}/edit/",
                         {"text": self.text_edit, "group": self.group.pk, 'image': img}, follow=True)
        pages = [f"/group/{self.slug}", f"/{self.user.username}/", f"/{self.user.username}/{self.post.pk}/"]
        for page in pages:
            response_page = self.client.get(page)
            self.assertIn(bytes("""<img class="card-img" """, encoding="UTF-8"), response_page.content)

    def test_not_image(self):
        with open('media/posts/1.txt', 'rb') as img:
            self.client.post(f"/{self.user.username}/{self.post.pk}/edit/",
                         {"text": self.text_edit, "group": self.group.pk, 'image': img}, follow=True)
        pages = [f"/group/{self.slug}", f"/{self.user.username}/", f"/{self.user.username}/{self.post.pk}/"]
        for page in pages:
    ...
    def test_cache(self):
        ...
        response_cache_post_edit_index = self.client.get("/")
...
class FollowCommentTest(TestCase):
    ...
    def test_follow(self):
        self.login_follow = self.client.login(username=self.username_follow, password=self.password_follow)
        response_follow = self.client.post(f"/{self.user.username}/follow/", {"username": self.user.username}, follow=True)
        ...
        response_unfollow = self.client.post(f"/{self.user.username}/unfollow/", {"username": self.user.username}, follow=True)
    ...
    def test_follow_index(self):
        self.login_follow = self.client.login(username=self.username_follow, password=self.password_follow)
        self.client.post(f"/{self.user.username}/follow/", {"username": self.user.username}, follow=True)
        response_follow = self.client.get("/follow/")
        self.assertContains(response_follow, self.text, count=None, status_code=200, html=False)
        self.login_not_follow = self.client.login(username=self.username_not_follow, password=self.password_not_follow)
        response_not_follow = self.client.get("/follow/")
        self.assertNotContains(response_not_follow, self.text, status_code=200, html=False)

    def test_comment(self):
        response_not_auth_comment = self.client.post(f"/{self.user.username}/{self.post.pk}/comment/", {"text": self.text}, follow=True)
        self.assertEqual(response_not_auth_comment.redirect_chain, [(f"/auth/login/?next=/{self.username}/1/comment/", 302)])
        self.login_follow = self.client.login(username=self.username_follow, password=self.password_follow)
        response_auth_comment = self.client.post(f"/{self.user.username}/{self.post.pk}/comment/", {"text": self.text}, follow=True)
        self.assertEqual(response_auth_comment.status_code, 200)
        response_post_comment = self.client.get(f"/{self.user.username}/{self.post.pk}/")
